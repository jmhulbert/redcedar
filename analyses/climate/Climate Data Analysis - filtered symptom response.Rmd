---
title: "Climate Data Analysis - filtered symptom response"
author: "Joey Hulbert"
date: "5/29/2022"
...
## Group response variables to filtered symptoms groups
...
```{r}
level_key <- c("Healthy" = "Healthy", "Thinning Canopy" = "Thinning Canopy", "New Dead Top (red or brown needles still attached)" = "Dead Top", "Old Dead Top (needles already gone)" = "Dead Top", "Tree is dead" = "Tree is Dead", "Multiple Symptoms (please list in Notes)" = "Other", "Extra Cone Crop" = "Other", "Browning Canopy" = "Other","Branch Dieback or 'Flagging'" = "Other", "Other (please describe in Notes)" = "Other", "Yellowing Canopy" = "Other")

reclassified <- combo.filtered.nearzerovar
reclassified$field.tree.canopy.symptoms <- recode_factor(reclassified$field.tree.canopy.symptoms, !!!level_key)
#levels(reclassified$field.tree.canopy.symptoms)
reclassified <- reclassified %>% filter(field.tree.canopy.symptoms!="Candelabra top or very old spike top (old growth)") %>% droplevels()
reclassified$field.tree.canopy.symptoms <- as.factor(reclassified$field.tree.canopy.symptoms)
```

```{r}
reclassified %>% group_by(field.tree.canopy.symptoms) %>% count()
```

```{r}
reclassified <- na.omit(reclassified)
```

reclassified tree health categories
...
* General approach
  + build full model using reclassified response grouping variable
    + reclassified response 
...
```{r}
monthless.reclassified <- reclassified[-c(9:155,232:378)]
```

```{r}
normal.monthless.reclassified <- reclassified[-c(9:378)]
```
...
```{r full rf model}
set.seed(71)
rf <- randomForest(field.tree.canopy.symptoms ~ ., data=normal.monthless.reclassified, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
```
