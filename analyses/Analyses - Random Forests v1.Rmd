 ...
 * Do following before next commit
    + Add 30yr normals into data (will need to rename all factors with 30y precidence probably).
    + calculate better variables (aspect, and those that combine both topo and climate data?) 
...
```{r}
combo.decade.filtered<- combo.decade %>% select(-c("field.optional...did.the.tree.have.heat.damage"
...
```{r}
# combo.decade.filtered <- combo.decade.filtered[c(45,50:59,68:332,337:341,355:394,396:503)]
```

```{r include=FALSE}
nearZeroVar(combo.decade.filtered)
combo.decade.filtered.nearzerovar <- combo.decade.filtered[,-nearZeroVar(combo.decade.filtered)]
#write.csv(combo.decade.filtered,file="./data/troubleshoot.csv")
```

```{r}
combo.decade.filtered.nearzerovar <-as.data.frame(unclass(combo.decade.filtered.nearzerovar),stringsAsFactors=TRUE) #change all chars to factors
```
...
for (n in names(combo.decade.filtered.nearzerovar))
  if (is.factor(combo.decade.filtered.nearzerovar[[n]])) {
    print(n)
    print(length(levels(combo.decade.filtered.nearzerovar[[n]])))
  }
```
