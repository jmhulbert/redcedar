...
```{r}
wasoils <- read.csv("./data/WA_GPS_Soils_1500_Extract.csv",header=TRUE)
orsoils <- read.csv("./data/OR_GPS_Soils_1500_Extract.csv",header=TRUE)
...
There are `nrow(combo.filtered)` observations from Oregon and Washington
...
First, remove columns without any data (entire column is NAs)
...
```{r}
less.fifty.three <- binary %>% select(-c("muaggatt_musym","muaggatt_muname","component_compname","component_geomdesc","component_taxclname","component_taxsubgrp","component_taxgrtgroup")) 
```
...
```{r}
summary(binary$component_taxsuborder)
```
...
```{r}
summary(binary$component_slopelenusle_r)
```
```{r}
summary(binary$component_irrcapcl)
```

```{r}
summary(binary$component_map_r)
```

```{r}
summary(binary$muaggatt_iccdcd)
```
